...
import {
  ...
  PestoContent,
} from './../../pesto-content-type/schemas/PestoContent.schema';
*/
...
export class PestoContent {
  ...
  @Prop({
    // ref: 'Ingredient',
    ref: 'PestoContentType',
    required: true, // you can't create a content without a pesto  content type.
    unique: false, // obviously several contents can be associated to the same  content type
    type: mongoose.Types.ObjectId, // this will be useful so that mongoose performs a format check, it must be a valid mongoose/mongoDB ID
    auto: false, // it is not auto generated, it will be an existing pesto content type id
  })
  content_type_id: mongoose.Types.ObjectId; // mongoose.Schema.Types.ObjectId;
